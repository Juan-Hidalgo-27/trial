...
def command_line_options():
    ...
    parser.add_argument(
        ...
        default=[0.05, 0.05],
        nargs="+",
        help="threshold for deficit simulation",
    )
    parser.add_argument(
        "--deficits",
        "--deficits",
        ...
        nargs="+",
        help="functional parcellation deficits, 1 to 16",
    )
    parser.add_argument(
        "--biasdegree",
        "--biasdegree",
        ...
        nargs="+",
        help="Degree of bias, 0 to 1",
    )
    parser.add_argument(
        "--biastype",
        "--biastype",
        type=str,
        default=["observed", "unobserved"],
        nargs="+",
        help="bias simulation method - by location or agnostic",
    )
    parser.add_argument(
        "--te",
        "--te",
    ...
    parser.add_argument(
        ...
        nargs="+",
        help="autoencoder bottleneck numbers of components",
    )
    parser.add_argument(
        "--simpleatlases",
        "--simpleatlases",
        ...
        default=[
            False,
            "all_territories",
            "major_arterial_territories",
            "major_arterial_territories_lat",
            "major_territories",
            "clusters_lat",
        ],
        nargs="+",
        help="simple atlases to loop through",
    )
    parser.add_argument(
        "--simpleatlas_argmaxs",
        "--simpleatlas_argmaxs",
        ...
        default=[True],
        nargs="+",
        help="One-hot encode simple atlas intersection or use dice score",
    )
    parser.add_argument(
        "--vols",
        "--vols",
        ...
        default=[True, False],
        nargs="+",
        help="include volume with simple atlas representations",
    )
    parser.add_argument(
        "--centroids",
        "--centroids",
        ...
        default=[False],
        nargs="+",
        help="Include location of lesion centroid with simple atlas representation",
    )
    ...
    parser.add_argument(
        ...
        "--ml_models",
        type=str,
        default=["logistic_regression", "extra_trees", "xgb"],
        nargs="+",
        help="sklearn-style classifiers to use",
    )
    ...
    parser.add_argument(
        ...
        type=bool,
        default=False,
        help="Use vae representation",
    )
    parser.add_argument(
        "--use_ae", "--use_ae", type=bool, default=False, help="Use ae representation"
    )
    parser.add_argument(
        "--use_nmf", "--use_nmf", type=bool, default=True, help="Use nmf representation"
    )
    parser.add_argument(
        "--use_pca", "--use_pca", type=bool, default=True, help="Use pca representation"
    )

    args = parser.parse_args()
    ground_truth_params = (
        args.k,
        args.gene_or_receptor,
        args.lesion_or_disconnectome,
        args.lesion_deficit_thresh,
        args.deficits,
        args.biasdegree,
        args.biastype,
        args.te,
    ...
    ml_models_dict = {}
    if "logistic_regression" in ml_models:
        ml_models_dict["logistic_regression"] = LogisticRegression()
    if "extra_trees" in ml_models:
        ml_models_dict["extra_trees"] = ExtraTreesClassifier()
    if "xgb" in ml_models:
        ml_models_dict["xgb"] = XGBClassifier()
    return (
        ground_truth_params,
        representation_params,
        selection,
...
def process_results(pred_outcome_given_W1, pred_outcome_given_W0, true_ITE):
    ...
    if len(true_ITE_xor) > 0:
        ...
        prescriptive_acc = (
            torch.sum(torch.diag(presc_conf_matrix)) / torch.sum(presc_conf_matrix)
        ).item()
        ...
        prescriptive_PEHE, prescriptive_acc, prescriptive_balacc = (
            torch.nan,
            torch.nan,
            torch.nan,
        )
        presc_conf_matrix = torch.Tensor(
    ...
    observed_conf_matrix = torch.Tensor(
        [[observed_tn, observed_fp], [observed_fn, observed_tp]]
    )
    observed_acc = (
        torch.sum(torch.diag(observed_conf_matrix)) / torch.sum(observed_conf_matrix)
    ).item()
    ...
    return (
        ...
        observed_acc,
        observed_balacc,
        observed_conf_matrix.cpu().detach().numpy().astype(int),
    ), (
        prescriptive_PEHE,
        prescriptive_acc,
        prescriptive_balacc,
        presc_conf_matrix.cpu().detach().numpy().astype(int),
    )
...
class ground_truth_simulation:
    ...
    def simulate_bias(self, train, biastype):
        ...
        if self.BIAS == 0:
            ...
            allocation.append(
                list(
                    uphalf.iloc[
                        int(np.round(len(uphalf) * (1 - proportion_fix))) :
                    ].index
                )
            )
            uphalf_iter = uphalf.iloc[
                : int(np.round(len(uphalf) * (1 - proportion_fix)))
            ]
            ...
            for i in range(len(uphalf_iter)):
                ...
                if (
                    np.random.choice(
                        [0, 1], p=[1 - uphalf_iter_probs[i], uphalf_iter_probs[i]]
                    )
                    == 1
                ):
                    allocation.append([inds[i]])
            ...
            if proportion_fix == 1:
                ...
                lowhalf_iter = lowhalf.iloc[
                    -int(np.round(len(lowhalf) * (1 - proportion_fix))) :
                ]
            ...
            for i in range(len(lowhalf_iter_probs)):
                ...
                if (
                    np.random.choice(
                        [0, 1], p=[1 - lowhalf_iter_probs[i], lowhalf_iter_probs[i]]
                    )
                    == 1
                ):
                    allocation.append([inds[i]])
    ...
    def simulate_trial(self, train, roi_centroids):
        ...
        if self.BIASTYPE == "unobserved":
            ...
            xyz = [
                abs(roi_centroids_x[0] - roi_centroids_x[1]),
        ...
        susceptible = train.loc[
            (train["group"] == train["y_true"]) | (train["y_true"] == 0.5)
        ]
        np.random.seed(self.seed)
        self.seed += 1
        respond = np.random.choice(
            susceptible.index, int(np.round(len(susceptible) * self.TE)), replace=False
        )
        ...
        if self.RE != 0:
            np.random.seed(self.seed)
            self.seed += 1
            spont_resp = np.random.choice(
                train.index, int(round(len(train) * self.RE)), replace=False
            )
...
class genGT:
    ...
    def generate_iteration(self):
        ...
        train = pd.read_pickle(
            os.path.join(
                loadpath,
                f"train_{input_type}_{self.lesion_deficit_thresh}_{self.k}.pkl",
            )
        )
        test = pd.read_pickle(
            os.path.join(
                loadpath, f"test_{input_type}_{self.lesion_deficit_thresh}_{self.k}.pkl"
            )
        )

        roi_centroids = np.array(
            pd.read_json(os.path.join(loadpath, "centroids.json"))[
                self.deficits
            ].tolist()
        )

        train_slice = train.loc[
            (train[f"{self.deficit_names[self.deficits]}_W0"] == 1)
            | (train[f"{self.deficit_names[self.deficits]}_W1"] == 1)
        ].reset_index()
        if len(train_slice) > 0:
            train_slice.loc[
                train_slice[f"{self.deficit_names[self.deficits]}_W1"] == 1, "y_true"
            ] = 1
            train_slice.loc[
                train_slice[f"{self.deficit_names[self.deficits]}_W0"] == 1, "y_true"
            ] = 0
            train_slice.loc[
                (train_slice[f"{self.deficit_names[self.deficits]}_W0"] == 1)
                & (train_slice[f"{self.deficit_names[self.deficits]}_W1"] == 1),
                "y_true",
            ] = 0.5
        else:
            train_slice = None
        ...
        test_slice = test.loc[
            (test[f"{self.deficit_names[self.deficits]}_W0"] == 1)
            | (test[f"{self.deficit_names[self.deficits]}_W1"] == 1)
        ].reset_index()
        if len(test_slice) > 0:
            test_slice.loc[
                test_slice[f"{self.deficit_names[self.deficits]}_W1"] == 1, "y_true"
            ] = 1
            test_slice.loc[
                test_slice[f"{self.deficit_names[self.deficits]}_W0"] == 1, "y_true"
            ] = 0
            test_slice.loc[
                (test_slice[f"{self.deficit_names[self.deficits]}_W0"] == 1)
                & (test_slice[f"{self.deficit_names[self.deficits]}_W1"] == 1),
                "y_true",
            ] = 0.5
        else:
            test_slice = None
    ...
    def prepare_representations(
        self, train_df, test_df, ae=True, vae=True, nmf=True, pca=True
    ):
        X_train, X_test = {}, {}
        if self.bottlenecks > 0:
            if ae:
                X_train["ae"] = (
                    np.stack(
                        train_df[
                            f"ae_{self.lesion_or_disconnectome}_{int(self.bottlenecks)}_K{self.k}"
                        ]
                    )
                    .astype(float)
                    .reshape(len(train_df), self.bottlenecks)
                )
                X_test["ae"] = (
                    np.stack(
                        test_df[
                    ...
                    .astype(float)
                    .reshape(len(test_df), self.bottlenecks)
                )
            ...
            if vae:
                X_train["vae"] = (
                    np.stack(
                        train_df[
                            f"vae_means_{self.lesion_or_disconnectome}_{int(self.bottlenecks)}_K{self.k}"
                        ]
                    )
                    .astype(float)
                    .reshape(len(train_df), self.bottlenecks)
            ...
            if nmf:
                X_train["nmf"] = (
                    np.stack(
                        train_df[
                            f"nmf_{self.lesion_or_disconnectome}_{int(self.bottlenecks)}_K{self.k}"
                        ]
                    )
                    .astype(float)
                    .reshape(len(train_df), self.bottlenecks)
            ...
            if pca:
                X_train["pca"] = (
                    np.stack(
                        train_df[
                            f"pca_{self.lesion_or_disconnectome}_{int(self.bottlenecks)}_K{self.k}"
                        ]
                    )
                    .astype(float)
                    .reshape(len(train_df), self.bottlenecks)
            ...
            if self.simpleatlases:
                X_train_simpleatlas = np.stack(
                    train_df[
                        f"{self.lesion_or_disconnectome}_{self.simpleatlases}"
                    ].to_numpy()
                )
                X_test_simpleatlas = np.stack(
                    test_df[
                        f"{self.lesion_or_disconnectome}_{self.simpleatlases}"
                    ].to_numpy()
                )
            ...
            if self.vols:
                X_train_vols = train_df[
                    f"{self.lesion_or_disconnectome}_vol"
                ].to_numpy()
                scaler = StandardScaler().fit(X_train_vols.reshape(-1, 1))
                X_train_vols = scaler.transform(X_train_vols.reshape(-1, 1))
                X_test_vols = scaler.transform(
                    np.stack(
                        test_df[f"{self.lesion_or_disconnectome}_vol"].to_numpy()
                    ).reshape(-1, 1)
                )
                vols_done = True
            ...
            if self.centroids:
                X_train_centroid_x = train_df[
                    f"{self.lesion_or_disconnectome}_centroid_x"
                ].to_numpy()
                X_train_centroid_y = train_df[
                    f"{self.lesion_or_disconnectome}_centroid_y"
                ].to_numpy()
                X_train_centroid_z = train_df[
                    f"{self.lesion_or_disconnectome}_centroid_z"
                ].to_numpy()
                X_train_centroids = np.vstack(
                    [X_train_centroid_x, X_train_centroid_y, X_train_centroid_z]
                ).T
                centroid_scaler = StandardScaler().fit(X_train_centroids)
                X_train_centroids = centroid_scaler.transform(X_train_centroids)
                ...
                X_test_centroids = np.vstack(
                    [X_test_centroid_x, X_test_centroid_y, X_test_centroid_z]
                ).T
                X_test_centroids = centroid_scaler.transform(X_test_centroids)
                centroids_done = True

            X_train_iter = np.zeros((len(train_df), 1))
            if simpleatlas_done:
                X_train_iter = np.hstack([X_train_iter, X_train_simpleatlas])
            if vols_done:
                X_train_iter = np.hstack([X_train_iter, X_train_vols])
            if centroids_done:
                X_train_iter = np.hstack([X_train_iter, X_train_centroids])
            X_train_iter = (
                X_train_iter[:, 1:] if X_train_iter.shape[1] > 1 else X_train_iter
            )
            X_train[
                (
                    self.simpleatlases,
            ...
            if simpleatlas_done:
                X_test_iter = np.hstack([X_test_iter, X_test_simpleatlas])
            if vols_done:
                X_test_iter = np.hstack([X_test_iter, X_test_vols])
            if centroids_done:
                X_test_iter = np.hstack([X_test_iter, X_test_centroids])
            X_test_iter = (
                X_test_iter[:, 1:] if X_test_iter.shape[1] > 1 else X_test_iter
            )
            X_test[
                (
                    self.simpleatlases,
        ...
        for BIASTYPE in self.biastype:
            for TE, RE in zip(self.te, self.re):
                for BIAS in self.biasdegree:
                    ...
                    gt_sim = ground_truth_simulation(
                        self.lesion_or_disconnectome, TE, RE, BIAS, BIASTYPE, self.k
                    )
...
def process_onemodel(
    train_embeddings, W_train, Y_train, test_embeddings, true_ITE, ml_models, xval_dict
):
    ...
    test_embeddings_W1, test_embeddings_W0 = np.hstack(
        [test_embeddings, np.ones([test_embeddings.shape[0], 1])]
    ), np.hstack([test_embeddings, np.zeros([test_embeddings.shape[0], 1])])
    ...
    for model_name in ml_models.keys():
        ...
        observed_results, prescriptive_results = process_results(
            torch.Tensor(pred_outcome_given_W1).view(-1, 1),
        ...
        results_onemodel = pd.concat(
            [results_onemodel, pd.DataFrame(onemodel_results, index=[0])]
        )
    return results_onemodel


def process_twomodels(
    train_embeddings, W_train, Y_train, test_embeddings, true_ITE, ml_models, xval_dict
):
    ...
    test_embeddings = test_embeddings.cpu().detach().numpy()
    X_given_W1_observed, Y_given_W1_observed = (
        train_embeddings[W_train == 1, :],
        Y_train[W_train == 1],
    )
    X_given_W0_observed, Y_given_W0_observed = (
        train_embeddings[W_train == 0, :],
        Y_train[W_train == 0],
    )
    true_ITE = true_ITE.cpu()
    ...
    for model_name in ml_models.keys():
        ...
        results_twomodels = pd.concat(
            [results_twomodels, pd.DataFrame(twomodel_results, index=[0])]
        )
...
def prepare_array_mapping(
    ...
    for K in k:
        for GENE_OR_RECEPTOR in gene_or_receptor:
            for LESION_OR_DISCONNECTOME, LESION_DEFICIT_THRESH in zip(
...
class prescription_processor:
    ...
    def run_iteration(self, train_data, test_data, xval_dict, X_train, X_test):
        ...
        if len(y_train) > self.min_n - 1:
            w_train, y_train = (
                torch.from_numpy(w_train).view(-1, 1).float(),
                torch.from_numpy(y_train).view(-1, 1).float(),
            )
            true_ITE = torch.Tensor(test_data).view(-1, 1)
            if bottleneck > 0:
                for rep in X_train.keys():
                    results_onemodel = process_onemodel(
                        X_train[rep],
                        w_train,
                        y_train,
                        X_test[rep],
                    ...
                    results_twomodels = process_twomodels(
                        X_train[rep],
                        w_train,
                        y_train,
                        X_test[rep],
                ...
                for params in X_train.keys():
                    ...
                    results_onemodel = process_onemodel(
                        ...
                        true_ITE,
                        self.ml_models,
                        {**xval_dict, "representation": "no_img"},
                    )
                    results_df = pd.concat([results_df, results_onemodel])
                    results_twomodels = process_twomodels(
                        X_train_iter,
    ...
    def run_all(self, iter_params, savepath):
        ...
        if not os.path.exists(savepath_grid):
            os.makedirs(savepath_grid)

        iter_params = pd.DataFrame(
            iter_params,
            columns=[
                "k",
                "gene_or_receptor",
        ...
        lesion_deficit_thresh = (
            iter_params["lesion_deficit_thresh"].to_numpy().astype(float)
        )
        ...
        simpleatlas_argmaxs = [
            True if x == "True" else False for x in simpleatlas_argmaxs
        ]
        ...
        if 0 not in bottlenecks:
            simpleatlases, simpleatlas_argmaxs = [False] * total_sims, [
                False
            ] * total_sims
        ...
        for (
            ...
            if train_df is not None and test_df is not None:
                X_train, X_test = prepare_generator.prepare_representations(
                    train_df,
                    test_df,
                    ae=self.run_ae,
                    vae=self.run_vae,
                ...
                for representation in X_train.keys():
                    X_train[representation] = torch.from_numpy(
                        X_train[representation]
                    ).float()
                    X_test[representation] = torch.from_numpy(
                        X_test[representation]
                    ).float()
                for (
                    train_data,
                    test_data,
                ), xval_dict in prepare_generator.simulate_observational_data(
                    train_df, test_df, roi_centroids
                ):
                    results_df = self.run_iteration(
                        train_data, test_data, xval_dict, X_train, X_test
                    )
    ...
    def run(self):
        iter_params = prepare_array_mapping(
            self.k,
            self.gene_or_receptor,
            self.lesion_or_disconnectome,
            self.lesion_deficit_thresh,
